...
router.get('/', async(req, res) => {
    try {
        const tickets = await schemas.Tickets.find().populate('client projet author');
...
router.get('/:id', async(req, res) => {
    try {
        const ticket = await schemas.Tickets.findById(req.params.id).populate('client projet author');
